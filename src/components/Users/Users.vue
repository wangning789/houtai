  ...
  <el-table-column
    ...
    <el-table-column
     ...
     <el-table-column
      ...
      align="center">
      <template slot-scope="scope">
       <el-switch v-model="scope.row.mg_state" @change="change(scope)">
</el-switch>
      </template>
    </el-table-column>
...
<el-dialog title="分配用户" :visible.sync="bution" width="30%" >
  ...
  <el-form ref="form" :model="vofform" >
  <el-form-item >
    <el-select v-model="vofform.region"  placeholder="请选择需要分配的角色">
...
const { mapState: userState, mapActions: userActions } = userModule;
 export default {
  ...
  // 编辑输入框验证状态设置
   ...
   methods: {
      ...userActions(["getusers","getusername","getaddusers",'getdelete',"getinformation",'getrole','getdistribution','getusersover']),
    ...
    change(val){
      this.getusersover({
        uId:val.row.id,
        type:val.row.mg_state,
          query:this.content,
       pagenum:this.pagenum,
       pagesize:this.pagesize
      })
      // console.log(val.row.mg_state)
    },
