<template>
  <div>
    ...
    >
      <template v-slot:new="scope">
        ...
        <el-button type="warning" @click="setCheckedKeys(scope)">分配角色</el-button>
      </template>
    </Role>
  ...
  <!-- 编辑弹出框 -->
    ...
    <Tree @handleClosetree="handleClosetree" @clickjur='clickjur' :ids="ids" :tree="tree" @clickjurisdiction='clickjurisdiction' :dialoging="dialoging"></Tree>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...userActions(["getrole", "getroleid", "getdeleterole",'getroleodr','getfenlei','getlodi','getquanxian','postgetroles']),
  ...
  setCheckedKeys(val){
    ...
    this.tableData.map((item,index)=>{
      if(val.scope.row.id===item.id&&item.children&&item.children.length>0){
