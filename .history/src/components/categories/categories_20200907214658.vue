      ...
      <template>
        <el-table :data="commoditytwo.result" style="width: 100%" border>
         ...
         <template v-for="item in msg" >
             <el-table-column v-if="item.label==='分类名称'" :key="item.cat_id" type="expand" v-if="item.label==='分类名称'"  :label="item.label"  :width="item.width">
               </el-table-column>
            <el-table-column :key="item.cat_id" :type="item.type" :label="item.label"  :width="item.width" ></el-table-column>
