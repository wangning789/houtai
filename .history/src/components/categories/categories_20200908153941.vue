    ...
    <el-card class="box-card">
      ...
      <el-dialog title="添加编辑" :visible.sync="dialog" width="30%" :before-close="addhandleClose">
        <el-form :model="roufrom" :rules="addrules" label-width="100px" class="demo-ruleForm">
          <el-form-item label="编辑名称" prop="name">
            <el-input @input="input" v-model="roufrom.name"></el-input>
          </el-form-item>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="addclicktrue" v-loading.fullscreen.lock="fullscreenLoading">确 定</el-button>
        </span>
      </el-dialog>
...
export default {
  ...
  methods: {
    ...userActions(["getcategoriesTwo", "getboydlt", "geteaitfenlei",'getcategories','getaddcategories']),
    handleClose(done) {
  ...
  addclicktrue(){
    ...
    let cat_pid=''
    let cat_level=''
    if(this.roufrom.label===""){
      cat_pid='0'
      cat_level='0'
    }
    if(this.roufrom.label.length>0&&this.roufrom.label){
        cat_level=JSON.stringify(this.roufrom.label.length)
        
    }
    if(this.roufrom.label.length===1&&this.roufrom.label){
      cat_pid=this.roufrom.label[0]
    }else if(this.roufrom.label.length===2&&this.roufrom.label){
      cat_pid=this.roufrom.label[1]
    }
    this.getaddcategories({
      cat_pid:cat_pid,
      cat_name:this.roufrom.name,
       cat_level: cat_level,
        type: this.type,
