<template>
        ...
        <template>
          ...
          <el-table-column align="center" label="是否付款" width>
            <template slot-scope="scope">
              <div v-if="scope.row.order_pay==='0'">
                <el-tag>已付款</el-tag>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" align="center" width>
            <template slot-scope="scope">
              <el-button type="primary" @click="eitclick(scope.row)">
    ...
    <el-dialog title="编辑地址" :visible.sync="dialog" width="30%" :before-close="addhandleClose">
      <el-form :model="from" label-width="100px" class="demo-ruleForm">
        <el-form-item label="城市" prop="region">
          <el-cascader clearable v-model="from.label" :options="city.data.children" :props="props"></el-cascader>
        </el-form-item>
      </el-form>
  ...
  <el-dialog title="物流信息" :visible.sync="dialogaox" width="30%" :before-close="addhandleClosebox">
    <el-steps :active="0" finish-status="success" direction="vertical">
      <div v-for="(item,index) in logistics" :key="index">
        <el-step :title="item.context" :description="item.ftime"></el-step>
      </div>
    </el-steps>
  </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...userActions(["getorders", "getlogistics"]),
    addhandleClosebox(){
      this.dialogaox=false
    },
