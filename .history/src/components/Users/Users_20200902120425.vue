...
const { mapState: userState, mapActions: userActions } = userModule;
 export default {
    ...
    //  自定义ipunt框验证状态
     var  validatePass=(rule, value, callback) => {
        if (value === '') {
          ...
          var iphone=this.addform.addipone
  ...
  // 编辑输入框验证状态设置
   ...
   methods: {
      ...userActions(["getusers","getusername","getaddusers"]),
    ...
    addConfirmedit(formName){
        this.$refs[formName].validate((valid) => {
          if (valid) {
            console.log("123",this.addform.addipone)
            this.getaddusers(
             this.addform.addusername,
              this.addform.addpassword,
              this.addform.addmailbox,
              this.addform.addipone
            )
