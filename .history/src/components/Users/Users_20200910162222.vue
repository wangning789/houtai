...
<el-dialog title="分配用户" :visible.sync="bution" width="30%" >
  ...
  <el-form-item >
    <el-select v-model="form.region" placeholder="请选择需要分配的角色">
      <template v-for="item in userdata">
        <el-option :key="item.id" @change="addinput(item)" :label="item.roleName" :value="item.id"></el-option>
      </template>
...
const { mapState: userState, mapActions: userActions } = userModule;
 export default {
  ...
  // 编辑输入框验证状态设置
   ...
   methods: {
      ...userActions(["getusers","getusername","getaddusers",'getdelete',"getinformation",'getrole','getdistribution']),
