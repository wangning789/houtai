          ...
          <template slot-scope="scope" v-if="scope.row.children&&scope.row.children.length>0">
            <div v-for="item in scope.row.children" :key="item.id">
              <div style="display:flex  ">
                <div class="onelabel">
                  <el-tag closable :disable-transitions="false" @close="handleClose(item.authName)"> {{item.authName}}</el-tag>
                </div>

               <div>
                  <div v-for="item1 in item.children" :key="item1.id">
                  <div class="twolabel"><el-tag closable type="success" :disable-transitions="false" @close="handleClose(item1.authName)"> {{item1.authName}}</el-tag></div>
                </div>
               </div>
              </div>
            </div>
